...
import { CreateRoomRequestSchema, UpdateRoomRequestSchema } from '@dnd-voice/shared';
...
export async function roomRoutes(fastify: FastifyInstance) {
  ...
  // Get single room
  fastify.get<{ Params: { id: string } }>('/api/rooms/:id', {
    preHandler: [authenticate],
  }, async (request, reply) => {
    try {
  ...
  fastify.put<{ Params: { id: string } }>('/api/rooms/:id', {
    preHandler: [authenticate, requireDM],
  }, async (request, reply) => {
    try {
      const { id } = request.params;
      ...
      if (existingRoom.created_by !== request.user!.id) {
        return reply.status(403).send({ error: 'Only the room creator can update this room' });
      }
  ...
  fastify.delete<{ Params: { id: string } }>('/api/rooms/:id', {
    preHandler: [authenticate, requireDM],
  }, async (request, reply) => {
    try {
      const { id } = request.params;
      ...
      if (existingRoom.created_by !== request.user!.id) {
        return reply.status(403).send({ error: 'Only the room creator can delete this room' });
      }
