  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      const rows = conversationQueries.getUserConversations.all(
        user.id,
        user.id,
        user.id, // CASE WHEN conditions
        user.id,
        user.id,
        user.id, // Unread count conditions
  ...
  fastify.post<{ Params: { userId: string } }>(
    ...
    async (request, reply) => {
      ...
      if (otherUserId === currentUser.id) {
        return reply
          .status(400)
          .send({ error: "Cannot start conversation with yourself" });
      }

      // Check if other user exists
      const userQueries = getUserQueries();
      const otherUser = userQueries.getById.get(otherUserId) as
        | UserRow
        | undefined;
      ...
      const fullConversation = conversationQueries.getById.get(
        conversation.id,
      ) as {
        id: number;
        user1_id: number;
      ...
      return reply.status(201).send({
        conversation: {
          ...
          otherUserId: isUser1
            ? fullConversation.user2_id
            : fullConversation.user1_id,
          otherUsername: isUser1
            ? fullConversation.user2_username
            : fullConversation.user1_username,
          otherRole: isUser1
            ? fullConversation.user2_role
            : fullConversation.user1_role,
        },
      });
    },
  );
