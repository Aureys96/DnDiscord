...
function setupTestSocketIO(httpServer: HTTPServer): SocketIOServer {
  ...
  io.on('connection', (socket) => {
    ...
    // Import voice channel functions
    const {
      addUserToVoice,
      removeUserFromVoice,
    ...
    socket.on('voice_join', ({ roomId }: { roomId: number }) => {
      if (isUserInVoice(roomId, user.id)) {
        socket.emit('voice_joined', { success: false, error: 'Already in voice channel' });
        return;
      }
      ...
      socket.to(`voice:${roomId}`).emit('voice_user_joined', {
        roomId,
        user: { userId: user.id, username: user.username, role: user.role, isMuted: false, isSpeaking: false },
      });
    });
    ...
    socket.on('voice_leave', ({ roomId }: { roomId: number }) => {
      ...
      socket.to(`voice:${roomId}`).emit('voice_user_left', { roomId, userId: user.id });
    });

    // Voice offer relay
    socket.on('voice_offer', ({ targetUserId, offer }: { targetUserId: number; offer: any }) => {
    ...
    // Voice answer relay
    socket.on('voice_answer', ({ targetUserId, answer }: { targetUserId: number; answer: any }) => {
      const targetSocket = [...io.sockets.sockets.values()].find(
        (s) => (s as any).user?.id === targetUserId
      );
      if (targetSocket) {
    ...
    // ICE candidate relay
    socket.on('voice_ice_candidate', ({ targetUserId, candidate }: { targetUserId: number; candidate: any }) => {
      const targetSocket = [...io.sockets.sockets.values()].find(
        (s) => (s as any).user?.id === targetUserId
      );
      if (targetSocket) {
        targetSocket.emit('voice_ice_candidate', { fromUserId: user.id, candidate });
      }
    });

    // Voice state update
    socket.on('voice_state_update', ({ roomId, isMuted }: { roomId: number; isMuted: boolean }) => {
      updateUserMuteState(roomId, user.id, isMuted);
      socket.to(`voice:${roomId}`).emit('voice_state_changed', { roomId, userId: user.id, isMuted });
    });

    // Voice speaking
    socket.on('voice_speaking', ({ roomId, isSpeaking }: { roomId: number; isSpeaking: boolean }) => {
      updateUserSpeakingState(roomId, user.id, isSpeaking);
      socket.to(`voice:${roomId}`).emit('voice_speaking_changed', { roomId, userId: user.id, isSpeaking });
    });
...
describe('Voice Socket Events', () => {
  ...
  const createToken = (user: { userId: number; username: string; role: 'dm' | 'player' }) => {
  ...
  describe('voice_join', () => {
    it('should allow user to join voice channel', async () => {
      ...
      const responsePromise = waitForEvent<{ success: boolean; voiceUsers: unknown[] }>(
        clientSocket1,
        'voice_joined'
      );
    ...
    it('should return existing users when joining', async () => {
      ...
      const join2Promise = waitForEvent<{ success: boolean; voiceUsers: unknown[] }>(
        clientSocket2,
        'voice_joined'
      );
    ...
    it('should broadcast voice_user_joined to other users in room', async () => {
      ...
      const broadcastPromise = waitForEvent<{ roomId: number; user: { userId: number } }>(
        clientSocket1,
        'voice_user_joined'
      );
  ...
  describe('voice_leave', () => {
    it('should allow user to leave voice channel', async () => {
      ...
      // Leave
      const leavePromise = waitForEvent<{ success: boolean }>(clientSocket1, 'voice_left');
  ...
  describe('voice_get_users', () => {
    it('should return list of voice users in room', async () => {
      ...
      // Request user list
      const usersPromise = waitForEvent<{ success: boolean; voiceUsers: unknown[] }>(
        clientSocket1,
        'voice_users'
      );
