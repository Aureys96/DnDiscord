        ...
        <div>
          <h2
            className={`text-sm font-semibold ${
              conversation.otherRole === 'dm' ? 'text-amber-500' : 'text-gray-100'
            }`}
          >
              ...
              return (
                ...
                >
                  ...
                  <div className={`w-8 flex-shrink-0 ${showAvatar ? '' : 'invisible'}`}>
                    {showAvatar && (
                      <div className="w-8 h-8 rounded-full bg-gray-700 flex items-center justify-center">
                        {(isOwnMessage ? user?.role : conversation.otherRole) === 'dm' ? (
                          <Crown className="w-4 h-4 text-amber-500" />
                        ) : (
                          <User className="w-4 h-4 text-violet-400" />
                        )}
                  ...
                  <div className={`max-w-[70%] ${isOwnMessage ? 'text-right' : ''}`}>
                    {showAvatar && (
                      <div className={`flex items-center gap-2 mb-1 ${isOwnMessage ? 'justify-end' : ''}`}>
                        ...
                        >
                          {isOwnMessage ? user?.username : conversation.otherUsername}
                        </span>
